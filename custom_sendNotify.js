...
async function sendNotify(text, desp, params = {}) {
    ...
    if (QYWX_AM_AY[2]) {
      ...
      user_qty = userIdTmp.length;
    }
    */
	let desp_all = desp.split("\n\n");
	for	(let i=0;i<desp_all.length && desp_all[i].replace(/[\n\r]/g, '');i++){
    	/*//失效管理已由update_global处理
   		if(desp_all[i].indexOf("重新登录获取") != -1){
        	//cookie失效
        	let userid = ChangeUserId(desp_all[i]);
        	try{
    			await db_query(`UPDATE jd set active = 0 WHERE userid='${userid}'`);
            }catch(e){}
        	console.log("用户"+userid+"因cookie无效将被暂停。")
        	desp_all[i] += "\n\ud83d\udd14您的京东管家因cookie无效将被暂停，请重新提交cookie后激活。"
        }
    	*/
    	if(desp_all[i].indexOf("重新登录获取") === -1){ //过滤cookie失效信息，由update_global统一管理
    		await sendNotify_call(text,desp_all[i],params);
        }
    }
}
...
//async function sendNotify(text, desp, params = {}) {
async function sendNotify_call(text, desp, params = {}) {
//Ethan Added=====End=====
  //提供6种通知
  //desp += `\n\n本脚本开源免费使用 By：https://gitee.com/lxk0301/jd_docker`; //Ethan Deleted
  await Promise.all([
...
function qywxamNotify(text, desp) {
  return new Promise(resolve => {
    if (QYWX_AM) {
      let desp_original = desp; //Ethan Inserted
      desp = desp.replace(/[帐|账]号[0-9]{1,2}|[帐|账]号 [0-9]{1,2}/g, `账号`); //Ethan Inserted
      desp = desp.replace(/签到号[0-9]{1,2}|签到号 [0-9]{1,2}/g, `京东账号`); //Ethan Inserted
      desp = desp.replace(/京东号[0-9]{1,2}|京东号 [0-9]{1,2}/g, `京东账号`); //Ethan Inserted
      const QYWX_AM_AY = QYWX_AM.split(',');
      ...
      $.post(options_accesstoken, (err, resp, data) => {
          ...
          case '0':
            options = {
              ...
              textcard: {
                title: `${text}`,
                description: `${desp}`,
                //url: 'https://github.com/lxk0301/jd_scripts', //Ethan Changed
                url: 'https://www.333234.xyz' //Ethan Changed
                //btntxt: '更多'//Ethan Changed
              }
            }
            break;
        ...
        options = {
          ...
          json: {
            //touser: `${ChangeUserId(desp)}`, //Ethan Changed
          	touser: `${ChangeUserId(desp_original)}`,
        ...
        $.post(options, (err, resp, data) => {
          try {
            if (err) {
              ...
              console.log('成员ID:' + ChangeUserId(desp_original) + '企业微信应用消息发送通知消息失败！！\n');
              console.log(err);
            } else {
              data = JSON.parse(data);
              if (data.errcode === 0) {
                //console.log('成员ID:' + ChangeUserId(desp) + '企业微信应用消息发送通知消息完成。\n'); //Ethan Changed
                console.log('成员ID:' + ChangeUserId(desp_original) + '企业微信应用消息发送通知消息完成。\n'); 
              } else {
                console.log(`${data.errmsg}\n`);
              }
            }
